...
sliderInput(inputId = "Schlafzimmer", 
            label = "Minimum Anzahl Schlafzimmer", 
            min = 0, 
            max = 12,
            value = 1)

# Preis pro Nacht
sliderInput(inputId = "Preis", 
            label = "Max Preis pro Nacht (log):", 
...
checkboxGroupInput(inputId = "Stadtteile", 
...
airbnb_dynamic() %>% 
  mutate(superhost = case_when(isTRUE(Host_superhost) ~ "Ja",
                          TRUE ~ "Nein"),
  ...
  addCircleMarkers(
    ...
    clusterOptions = markerClusterOptions(spiderfyOnMaxZoom = F, disableClusteringAtZoom = 15)) %>% 
  # Füge Legende hinzu
